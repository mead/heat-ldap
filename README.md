...
Nominally the steps are as follows.  Hostnames are autogenerated but follow
a sane logic, have a look at /etc/hosts (which exists everywhere) and you'll see.
If you changed the number of osds per node from 4, then these won't work.
...
By default the ceph journal size is 5gb, that won't fit onto our little volumes.
you should edit the local ceph.conf and add `osd_journal_size = 100` before creating the osd nodes;

1. `ceph-deploy disk zap storage-{0,1,2,3,4,5,6}-euk:/dev/vd{b,c,d,e}`  
1. `ceph-deploy osd create storage-{0,1,2,3,4,5,6}-euk:/dev/vd{b,c,d,e}`

Or if you want to, you can omit the create step and run prepare and activate steps individually.
